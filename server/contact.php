...
function verify_recaptcha(string $captchaResponse): bool
{
    ...
    if (!has_effective_value(RECAPTCHA_SECRET) || strlen(RECAPTCHA_SECRET) < 30) {
        error_log('reCAPTCHA secret nao configurada corretamente.');
        return false;
    }
...
function send_via_resend(
    ...
    $htmlBody = '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif;line-height:1.6;color:#333;margin:0;padding:0;background:#f5f5f5;">'
        ...
        . '<div style="background:linear-gradient(135deg,#2563eb,#1d4ed8);color:#fff;padding:30px 20px;text-align:center;">'
        ...
        . '<h2 style="margin:0 0 15px;font-size:14px;font-weight:600;color:#2563eb;text-transform:uppercase;letter-spacing:1px;">Informações de Contato</h2>'
        . '<div style="padding:12px;background:#f9f9fb;border-left:4px solid #2563eb;margin-bottom:10px;"><p style="margin:0;font-size:13px;color:#666;"><strong>Nome:</strong> ' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '</p></div>'
        . '<div style="padding:12px;background:#f9f9fb;border-left:4px solid #2563eb;margin-bottom:10px;"><p style="margin:0;font-size:13px;color:#666;"><strong>Email:</strong> <a href="mailto:' . htmlspecialchars($email, ENT_QUOTES, 'UTF-8') . '" style="color:#2563eb;">' . htmlspecialchars($email, ENT_QUOTES, 'UTF-8') . '</a></p></div>'
        . '<div style="padding:12px;background:#f9f9fb;border-left:4px solid #2563eb;margin-bottom:10px;"><p style="margin:0;font-size:13px;color:#666;"><strong>Telefone:</strong> ' . ($phone !== '' ? htmlspecialchars($phone, ENT_QUOTES, 'UTF-8') : '<em style="color:#999;">Não informado</em>') . '</p></div>'
        . '<div style="padding:12px;background:#f9f9fb;border-left:4px solid #2563eb;"><p style="margin:0;font-size:13px;color:#666;"><strong>Serviço:</strong> ' . $serviceDisplay . '</p></div>'
        . '<div style="margin-top:25px;padding-top:20px;border-top:1px solid #e5e5e5;">'
        . '<h2 style="margin:0 0 15px;font-size:14px;font-weight:600;color:#2563eb;text-transform:uppercase;letter-spacing:1px;">Mensagem</h2>'
...
try {
    ...
    json_error('Falha ao enviar agora. Tente novamente em instantes ou chame no WhatsApp.', 502, [
        'delivery' => 'failed',
    ]);
}